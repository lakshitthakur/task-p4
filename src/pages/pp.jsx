...
function PostPage() {
  // 1. React State Hook to handle conditional rendering between 'question' and 'article'
  const [postType, setPostType] = useState('question');

  // State management for Question fields
  ...
  // State management for Article fields
  ...
  // UI state for messages and post submission status
  const [errorMessage, setErrorMessage] = useState('');
  const [postSuccess, setPostSuccess] = useState(false);

  // Input change handler for the Question form
  const handleQuestionChange = (e) => {
    setQuestionData({ ...questionData, [e.target.name]: e.target.value });
  };

  // Input change handler for the Article form
  const handleArticleChange = (e) => {
    setArticleData({ ...articleData, [e.target.name]: e.target.value });
  };

  // Handle post type switch and clear past banners
  ...
  // Data validation before accepting submission
  const validateForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // Show required 'Post Received' notification (backend storage deferred to future task)
    setPostSuccess(true);
